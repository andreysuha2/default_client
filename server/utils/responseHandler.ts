...
export const defaultResponseHandler = <T extends EventHandlerRequest, D> (handler: EventAuthHandler<T, D>): EventHandler<T, D> => defineEventHandler<T>(async (event): Promise<D> => {
    try {
