<template>
    <UModal v-model:open="open"
            :title="t('pages.users.forms.remove.title', { name: user?.name })">
        <template #body>
                ...
                <UForm :schema="schema" :state="state" ref="form" @submit="onSubmit">
                    <app-row>
                        <app-cell class="flex justify-center">
                            <UButton color="error"
                                type="submit">{{ t('pages.users.forms.remove.submitBtn') }}</UButton>
                        </app-cell>
                        <app-cell class="flex justify-center">
                            <UButton variant="outline"
                                @click="open = false">{{ t('pages.users.forms.remove.cancelBtn') }}</UButton>
                        </app-cell>
                    </app-row>
                </UForm>
            </app-loader>
...
const { handleFormError } = useServerErrorHandler(),
    onSubmit = async (e: FormSubmitEvent<Schema>): Promise<void> => {
        if (!user) return;
        loader.value = true;
        users.remove(e.data.id).then((user) => {
            emit("user-removed", user);
            open.value = false
            toast.add({
                title: t("pages.users.forms.remove.toasts.success.title"),
                description: t("pages.users.forms.remove.toasts.success.description", { name: user.name }),
